$(document).ready(function() {
  ...
  getAllAttractions(function(results) {
    ...
    // add empty option
    let $option = $('<option>').attr("label", "Select an attraction...").attr("disabled","disabled").attr("selected","selected");;
  ...
  $("#select-attraction").on('change', function() {
    ...
    getAttractionSlots(val, function(results) {
      ...
      results.forEach(slot => {
        ...
        $("#attraction-capacity").text((100 - (currCapacity / totalCapacity) * 100).toString());
        let $li = $('<li>').text(`${slot.label} (${slot.capacity - slot.waiting}/${slot.capacity})`);
